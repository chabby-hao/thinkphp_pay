...
class MicroPay
{
	...
	public function pay($microPayInput)
	{
		...
		$subMchId = $microPayInput->getSubMchId();
		$subAppid = $microPayInput->getSubAppid();
		...
		while($queryTimes > 0)
		{
		    $queryTimes++;
			$succResult = 0;
			$queryResult = $this->query($out_trade_no, $succResult, $subMchId, $subAppid);
			//如果需要等待1s后继续
			if($succResult == 3){
				sleep(1);
				continue;
				...
				return $queryResult;
			} else if($succResult == 2){//用户支付中，输入密码
			    return 2;
			} 
			else {//订单交易失败
				return false;
	 ...
	 */
	public function query($out_trade_no, &$succCode, $subMchId, $subAppid)
	{
		$queryOrderInput = new WxPayOrderQuery();
		$queryOrderInput->SetOut_trade_no($out_trade_no);
		$queryOrderInput->setSubMchId($subMchId);
		$queryOrderInput->setSubAppid($subAppid);
